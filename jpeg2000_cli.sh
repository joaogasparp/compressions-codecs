...
if [ $# -eq 0 ]; then
    ...
    OUTPUT_DIR="compressed_images_bash/$DATASET/jpeg2000"
    mkdir -p "$OUTPUT_DIR"
    ...
    INPUT_FILES=("$@")
    OUTPUT_DIR="compressed_images_bash/${COMPRESS_DATASET:-INT}/jpeg2000"
    mkdir -p "$OUTPUT_DIR"
fi

TEMP_DIR="temp_pgm"
JPEG2000_BIN="jpeg2000/build/bin/grk_compress"
JPEG2000_DECOMP="jpeg2000/build/bin/grk_decompress"
RAW_TO_PGM="./raw_to_pgm"

mkdir -p "$TEMP_DIR"

if [ ! -x "$JPEG2000_BIN" ]; then
    echo "Error: $JPEG2000_BIN not found. Build with: ./compile_jpeg2000.sh"
    exit 1
fi
...
for input_file in "${INPUT_FILES[@]}"; do
    ...
    output_file="$OUTPUT_DIR/${base}.jp2"
    ...
    time_output=$( (/usr/bin/time -f "%e %M" "$JPEG2000_BIN" -i "$temp_pgm" -o "$output_file") 2>&1 | tail -1 )
    ...
    if [ -f "$output_file" ]; then
        ...
        temp_decomp="/tmp/jpeg2000_decomp_$$.pgm"
        "$JPEG2000_DECOMP" -i "$output_file" -o "$temp_decomp" > /dev/null 2>&1
        decomp_end=$(date +%s.%N)
        decomp_time=$(echo "$decomp_end - $decomp_start" | bc -l)
        
        temp_orig_data="/tmp/jpeg2000_orig_data_$$.bin"
        temp_decomp_data="/tmp/jpeg2000_decomp_data_$$.bin"
        tail -c +$(grep -abo '65535' "$temp_pgm" | head -1 | cut -d: -f1) "$temp_pgm" > "$temp_orig_data" 2>/dev/null
        tail -c +$(grep -abo '65535' "$temp_decomp" | head -1 | cut -d: -f1) "$temp_decomp" > "$temp_decomp_data" 2>/dev/null
