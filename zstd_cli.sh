...
if [ $# -eq 0 ]; then
    ...
    OUTPUT_DIR="compressed_images_bash/$DATASET/zstd"
    mkdir -p "$OUTPUT_DIR"
    ...
    INPUT_FILES=("$@")
    OUTPUT_DIR="compressed_images_bash/${COMPRESS_DATASET:-INT}/zstd"
    mkdir -p "$OUTPUT_DIR"
fi
...
for input_file in "${INPUT_FILES[@]}"; do
    ...
    time_output=$( (/usr/bin/time -f "%e %M" zstd -19 -k -f -q -o "$output_file" "$input_file") 2>&1 )
    ...
    if [ -f "$output_file" ]; then
        ...
        temp_decomp="/tmp/zstd_decomp_$$.raw"
        unzstd -c "$output_file" > "$temp_decomp" 2>&1
        decomp_end=$(date +%s.%N)
