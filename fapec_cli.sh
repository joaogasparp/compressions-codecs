...
if [ $# -eq 0 ]; then
    ...
    OUTPUT_DIR="compressed_images_bash/$DATASET/fapec"
    mkdir -p "$OUTPUT_DIR"
    ...
    INPUT_FILES=("$@")
    OUTPUT_DIR="compressed_images_bash/${COMPRESS_DATASET:-INT}/fapec"
    mkdir -p "$OUTPUT_DIR"
fi

FAPEC_BIN="fapec/fapec"
...
if [ ! -x "$FAPEC_BIN" ]; then
    echo "Error: $FAPEC_BIN not found or is not executable"
    exit 1
fi

if [ -f "fapec/fapeclic.dat" ]; then
    mkdir -p ~/.fapec
    cp -f fapec/fapeclic.dat ~/.fapec/ 2>/dev/null
fi

for input_file in "${INPUT_FILES[@]}"; do
    filename=$(basename "$input_file")
    base="${filename%.raw}"
    output_file="$OUTPUT_DIR/${base}.fapec"
    ...
    time_output=$( (/usr/bin/time -f "%e %M" bash -c "unset FAPEC_HOME && $FAPEC_BIN -qq -dtype 16 -be -dwt $width $height 1 0 16 2 -bl 512 -o '$output_file' '$input_file'" < /dev/null) 2>&1 | tail -1 )
    ...
    if [ -f "$output_file" ]; then
        ...
        temp_decomp="/tmp/fapec_decomp_$$.raw"
        timeout 60 bash -c "unset FAPEC_HOME && fapec/unfapec -o '$temp_decomp' '$output_file'" < /dev/null > /dev/null 2>&1
        decomp_end=$(date +%s.%N)
