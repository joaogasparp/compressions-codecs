...
if [ $# -eq 0 ]; then
    ...
    OUTPUT_DIR="compressed_images_bash/$DATASET/rice"
    mkdir -p "$OUTPUT_DIR"
    if [ ! -d "$FITS_DIR" ]; then
    ...
    INPUT_FILES=()
    OUTPUT_DIR="compressed_images_bash/${COMPRESS_DATASET:-INT}/rice"
    mkdir -p "$OUTPUT_DIR"
...
for fits_file in "${INPUT_FILES[@]}"; do
    ...
    base="${filename%.fits}"
    output_file="$OUTPUT_DIR/${base}.rice"
    ...
    time_output=$( (/usr/bin/time -f "%e %M" fpack -r "$fits_file") 2>&1 )
    ...
    if [ -f "$compressed_fits" ]; then
        ...
        temp_decomp="/tmp/rice_decomp_$$.fits"
        funpack -O "$temp_decomp" "$output_file" > /dev/null 2>&1
        decomp_end=$(date +%s.%N)
        decomp_time=$(echo "$decomp_end - $decomp_start" | bc -l)
        
        temp_orig_data="/tmp/rice_orig_data_$$.bin"
        temp_decomp_data="/tmp/rice_decomp_data_$$.bin"
