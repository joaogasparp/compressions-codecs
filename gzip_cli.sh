...
if [ $# -eq 0 ]; then
    ...
    OUTPUT_DIR="compressed_images_bash/$DATASET/gzip"
    mkdir -p "$OUTPUT_DIR"
    ...
    INPUT_FILES=("$@")
    OUTPUT_DIR="compressed_images_bash/${COMPRESS_DATASET:-INT}/gzip"
    mkdir -p "$OUTPUT_DIR"
fi
...
for input_file in "${INPUT_FILES[@]}"; do
    ...
    time_output=$( (/usr/bin/time -f "%e %M" gzip -9 -k -f -c "$input_file" > "$output_file") 2>&1 )
    ...
    temp_decomp="/tmp/gzip_decomp_$$.raw"
    gunzip -c "$output_file" > "$temp_decomp" 2>&1
    decomp_end=$(date +%s.%N)
