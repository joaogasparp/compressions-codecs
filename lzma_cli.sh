...
if [ $# -eq 0 ]; then
    ...
    OUTPUT_DIR="compressed_images_bash/$DATASET/lzma"
    mkdir -p "$OUTPUT_DIR"
    ...
    INPUT_FILES=("$@")
    OUTPUT_DIR="compressed_images_bash/${COMPRESS_DATASET:-INT}/lzma"
    mkdir -p "$OUTPUT_DIR"
fi
...
for input_file in "${INPUT_FILES[@]}"; do
    ...
    time_output=$( (/usr/bin/time -f "%e %M" xz -9 -k -f -c "$input_file" > "$output_file") 2>&1 )
    ...
    temp_decomp="/tmp/lzma_decomp_$$.raw"
    unxz -c "$output_file" > "$temp_decomp" 2>&1
    decomp_end=$(date +%s.%N)
