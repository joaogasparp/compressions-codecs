...
if [ $# -eq 0 ]; then
    ...
    OUTPUT_DIR="compressed_images_bash/$DATASET/jpegls"
    mkdir -p "$OUTPUT_DIR"
    ...
    INPUT_FILES=("$@")
    OUTPUT_DIR="compressed_images_bash/${COMPRESS_DATASET:-INT}/jpegls"
    mkdir -p "$OUTPUT_DIR"
fi

JPEGLS_BIN="jpegls_0.06.20120206/rawnjl2"

if [ ! -x "$JPEGLS_BIN" ]; then
    echo "Error: $JPEGLS_BIN not found. Build with: cd jpegls_0.06.20120206 && make"
    exit 1
fi
...
for input_file in "${INPUT_FILES[@]}"; do
    ...
    base="${filename%.raw}"
    output_file="$OUTPUT_DIR/${base}.jls"
    ...
    time_output=$( (/usr/bin/time -f "%e %M" "$JPEGLS_BIN" \
        -rows "$height" \
        -columns "$width" \
        -depth 16 \
        -input-endian big \
    ...
    if [ -f "$output_file" ]; then
        ...
        temp_decomp="/tmp/jpegls_test_$$.raw"
        "$JPEGLS_BIN" -d -output-endian big "$output_file" "$temp_decomp" > /dev/null 2>&1
        decomp_end=$(date +%s.%N)
